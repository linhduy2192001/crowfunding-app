...
const ImageUpload = ({ onChange = () => {}, name = "" }) => {
  const handleUploadImage = async (e) => {
    ...
    if (!imageData) {
      toast.error("Can't not upload image to imgdbApi");
    }
    const imageObj = {
  ...
  return (
    <label className="block w-full rounded-xl border border-dashed cursor-pointer flex items-center justify-center border-gray-200 h-[200px]">
